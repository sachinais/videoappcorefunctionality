package com.nick.sampleffmpeg.utils.svgandroid;
...
//            Log.d(TAG, "Float parsing '" + name + "=" + v + "'");
    ...
    private static class Properties {
        ...
        public Long getHexLong(String name) {
            String v = getAttr(name);
            if (v == null || !v.startsWith("#")) {
                return null;
            } else {
                try {
                    return Long.parseLong(v.substring(1), 16);
                } catch (NumberFormatException nfe) {
                    // todo - parse word-based color here
                    return null;
                }
    ...
    private static class SVGHandler extends DefaultHandler {
        ...
        private boolean doFill(Properties atts, HashMap<String, Shader> gradients) {
            ...
            if (fillString != null && fillString.startsWith("url(#")) {
                ...
                Long longColor = atts.getHexLong("fill");
                if (longColor != null) {
                    doColor(longColor);
                    paint.setStyle(Paint.Style.FILL);
                    return true;
        ...
        private void doColor(Long color) {
            long B = color & 0xFF;
            long G = (color >> 8) & 0xFF;
